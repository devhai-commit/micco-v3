...
async def get_allowed_document_ids(
    ...
    # Chỉ filter approval_status khi tính năng approvals được bật
    if settings.COMPAT_LEGACY_APPROVALS_ENABLE:
...
async def get_kg_graph(
    ...
    try:
        data = await kg.get_graph_data(
            ...
            max_depth=max_depth,
            max_nodes=max_nodes,
            allowed_doc_ids=None,  # graph view không filter theo document — user đã authenticated
        )
